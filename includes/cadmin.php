...
function config_form() {
	...
	if(isset($vars['phpcid']))
        ...
        $form = tag('form', attributes("action=\"$phpc_script\"",'method="post"','enctype="multipart/form-data"'),
			$hidden_div,
...
function user_list()
{
	...
	foreach ($users as $user) {
		...
		$tbody->add(tag('tr',
					...
					tag('td', create_checkbox("modify{$user['uid']}", "1", !empty($user['modify']), "<span class='button modificar'>".__('Modify')."</span>")),

...
$phpcdb = new PhpcDatabase(SQL_HOST, SQL_USER, SQL_PASSWD, SQL_DATABASE, SQL_PORT);
...
$phpcdb->get_calendars(); /*buscamos los calendario en la BD para poder asignar el caledario por default en phpcdatabase.php ln 541*/
...
if(empty($phpc_cal)) {
	...
	$phpc_cal = $phpcdb->get_calendar($phpcid);
	if(empty($phpc_cal)){
		phpc_do_logout(); /*como no hay calendario entonces deslogueamos al usuario*/

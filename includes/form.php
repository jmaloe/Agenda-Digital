 ...
 * sentences long at most
 */
class FormAdjuntarArchivo extends FormAtomicQuestion {
        
		var $type;

        function __construct($qid, $subject, $description = false,$maxlen = false, $required = false) {
		parent::__construct();
                $this->qid = $qid;
                $this->subject = $subject;
                $this->description = $description;
...
class FormSimpleImage extends FormAtomicQuestion{
        ...
        protected function get_specific_html($parent, $defaults) {
            if(!empty($defaults[$this->qid]))
            {
                return '<a href="'.$defaults[$this->qid].'" target="_blank"><img src="static/images/ico_archivo_adjunto.png"></a>';
            }
        }
...
class Form extends FormGroup {
	...
	var $hidden = array();
        ...
        function get_form($defaults = array()) {
                $form_attrs = attrs("action=\"{$this->action}\"",'method="POST"','enctype="multipart/form-data"');

...
function display_form() {
	...
	$when_group->add_part(new FormDateTimeQuestion('start',	"Del", $hour24, $date_format));
	$when_group->add_part(new FormDateTimeQuestion('end', "Hasta", $hour24, $date_format));
	...
	/*campos implementados, el 3er parametro indica la longitud maxima en el input*/
	$form->add_part(new FormFreeQuestion('lugar', 'Lugar',false,255,true));
	$form->add_part(new FormFreeQuestion('invitados', 'Invitados',false,65535,true));
	if($phpc_cal->can_create_readonly())
		$form->add_part(new FormCheckBoxQuestion('readonly', false,	__('Read-only')));
	$form->add_part(new FormAdjuntarArchivo('archivo_adjunto', 'Adjuntar archivo',false,0,false));
	$form->add_part(new FormSimpleImage('adjunto'));
...
function uploadArchivoAdjunto()
{
 try
 {
   //global $phpcdb;
   if(isset($_FILES['archivo_adjunto']) & !empty($_FILES['archivo_adjunto']))
   {	 	
	 	$elTmpName=$_FILES['archivo_adjunto']['tmp_name'];	 	

		$info = pathinfo($_FILES['archivo_adjunto']['name']);
		$ext = $info['extension']; //obtenemos la extensión del archivo
		
		$target = date("Ymd-His").".".$ext;
...
function process_form()
{
	...
	if(!isset($vars['eid'])) {
		...
		$eid = $phpcdb->create_event($phpcid, $phpc_user->get_uid(),
				...
				$readonly, $catid,$vars["lugar"],$vars["invitados"], uploadArchivoAdjunto());
	} else {
		$modify = true;
		$eid = $vars['eid'];
		$phpcdb->modify_event($eid, $vars['subject'], $vars['description'], $readonly, $catid,$vars["lugar"],$vars["invitados"], uploadArchivoAdjunto());
		if($modify_occur)
	...
	if($phpcid)
	{
	 global $phpc_mailer_path, $phpc_includes_path;

	 require_once("$phpc_mailer_path/Mailer.php");
	 require_once("$phpc_includes_path/display_event.php");

	 $correo = new Correo();
	 $correo->setAsunto("EVENTO: ".$vars['subject']);
	  //echo "EVENTO: ".$vars['subject'];
	 $correo->setRemitente($phpc_user->get_email(),$phpc_user->get_nombre_completo());
	  //echo "FROM:".$phpc_user->get_email()." - ".$phpc_user->get_nombre_completo();
	 
	 $contenido = display_event_by_eid($eid,false);
	  //echo "CONTENIDO:".$contenido->toString();
	 $correo->setContenido($contenido->toString()); /*false para no retornar los controles o botones: display_event.php*/ 
	 $direcciones = $phpcdb->getUsuariosPorCalendario($phpcid);
	  //print_r($direcciones);	 
	...
	if($eid != 0)
	{
		...
		return message_redirect(tag('', $message, create_event_link($eid, 'display_event', $eid)), "$phpc_script?action=display_event&phpcid=$phpcid&eid=$eid");
	} 
	else
	{

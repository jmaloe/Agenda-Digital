...
function admin() {
	global $phpc_version, $phpc_prefix;
    ...
    if($_SESSION["{$phpc_prefix}uid"]<4){
    	$menu = tag('ul',
			tag('li', tag('a', attrs('href="#phpc-admin-calendars"'),__('Calendars'))),
			tag('li', tag('a', attrs('href="#phpc-admin-users"'),__('Users'))),
			tag('li', tag('a', attrs('href="#phpc-admin-import"'),__('Import'))),
			tag('li', tag('a', attrs('href="#phpc-admin-translate"'),__('Translate'))));
	...
	$menu = tag('ul',
			tag('li', tag('a', attrs('href="#phpc-admin-calendars"'),__('Calendars'))),
			tag('li', tag('a', attrs('href="#phpc-admin-users"'),__('Users'))));
		return tag('', tag('div', attrs('class="phpc-tabs"'), $menu, calendar_list(), user_list()), $version);
	}
}
...
function calendar_list()
{
	global $phpc_script, $phpcdb;
        ...
        foreach($phpcdb->get_calendars() as $calendar) {
                ...
                $tbody->add(tag('tr',
                                tag('th', $title),
                                tag('td',create_action_link("<span class='button modificar'>".__("Edit")."</span>",
						"cadmin", array("phpcid" => $cid)),
					" ", create_action_link("<span class='button borrar'>".__("Delete")."</span>",
						"calendar_delete",
						array("cid" => $cid)))));
        }
...
function user_list()
{
	global $phpc_script, $phpcdb;
		...
		$tbody->add(tag('tr', tag('th', $user->username),
					tag('td', $groups),
					tag('td', create_action_link("<span class='button modificar'>".__("Edit Groups")."</span>", "user_groups", array("uid" => $user->uid))),
					tag('td', create_action_link("<span class='button borrar'>".__("Delete")."</span>",
							"user_delete",
							array("uid" => $user->uid)))));
	}

	$create_link = create_action_link('CREAR/MODIFICAR USUARIO',
			'user_create');

...
class PhpcDatabase {
	...
	private function get_event_fields() {
		...
		$fields = array('subject', 'description', 'owner', 'eid', 'cid',
				'readonly', 'catid','lugar','invitados','archivo_adjunto');
	...
	private function get_user_fields() {
		...
		return "`$users_table`.`uid`, `username`, `password`, `$users_table`.`admin`, `password_editable`, `timezone`, `language`,`nombre_completo`,`email`";
	}
	...
	function get_occurrences_by_date_range($cid, $from, $to)
	{
		...
		$query = "SELECT " . $this->get_occurrence_fields()
			.", `username`,`nombre_completo`, `name`, `bg_color`, `text_color`\n"
			."FROM `$events_table`\n"
	...
	function get_calendars_by_user($uid){
		/*if(!empty($this->calendars))
			return $this->calendars;*/
		$calendario;// = new Array();	
		//echo "USUARIO:".$uid;		
		$query = "SELECT c.* FROM phpc_users u, phpc_groups g, phpc_user_groups ug, phpc_calendars c WHERE u.uid=ug.uid and g.gid=ug.gid and g.cid=c.cid and u.uid=$uid";

		$sth = $this->dbh->query($query) or $this->db_error(__('Could not get calendars.'),	$query);

		while($result = $sth->fetch_assoc()) {			
			$cid = $result["cid"];
			//echo "DATO:".$cid;
			//if(empty($this->calendars[$cid]))
				$calendario[$cid] = new PhpcCalendar($result);
		}
	...
	function get_calendars() {
		...
		$query = "SELECT * "	."FROM `" . SQL_PREFIX .  "calendars`\n"; /*consulta por default cuando no hay usuario*/

		global $phpc_prefix;
		
		if(!empty($_SESSION["{$phpc_prefix}uid"])){
			$uid = $_SESSION["{$phpc_prefix}uid"];
			$query = "SELECT c.* FROM ".SQL_PREFIX."users u, ".SQL_PREFIX."groups g, ".SQL_PREFIX."user_groups ug, ".SQL_PREFIX."calendars c WHERE u.uid=ug.uid and g.gid=ug.gid and g.cid=c.cid and u.uid=$uid";			
		}
		...
		while($result = $sth->fetch_assoc()) {
			...
			if($cont==0){
				global $default_calendar_id;
				$default_calendar_id=$cid;
				$cont++;
			}

			if(empty($this->calendars[$cid]))
				$this->calendars[$cid] = new PhpcCalendar($result);		  
		}
	...
	{
		$query = "SELECT " .$this->get_user_fields()." FROM ".SQL_PREFIX."users WHERE username='$username'";
	...
	function create_user($username, $password, $make_admin, $nombre, $email) {
		$admin = $make_admin ? 1 : 0;
		$query = "INSERT into `".SQL_PREFIX."users`(`username`, `password`, `admin`,`nombre_completo`,`email`) VALUES('$username', '$password', $admin, '$nombre', '$email')";

		$this->dbh->query($query) or $this->db_error(__('Error creating user.'), $query);
	...
	function actualizarUsuario($uid, $username, $password, $isadmin, $nombrecompleto, $email){		
		$query = "UPDATE `" . SQL_PREFIX . "users` SET `username`='$username', `password`='$password',`admin`=$isadmin,`nombre_completo`='$nombrecompleto',`email`='$email' WHERE `uid`='$uid'";
		if($password=="SC")
			$query = "UPDATE `" . SQL_PREFIX . "users` SET `username`='$username', `admin`=$isadmin,`nombre_completo`='$nombrecompleto',`email`='$email' WHERE `uid`='$uid'";

		$this->dbh->query($query) or $this->db_error(__('Error updating password.'), $query);
		return true;
	}
	...
	function quitar_grupos($uid){
		$user_groups_table = SQL_PREFIX . 'user_groups';

		/*Primero quitamos todos los grupos a los que haya sido agregado al usuario*/

		$query = "DELETE FROM `$user_groups_table` WHERE uid=$uid";

		$this->dbh->query($query) or $this->db_error(__('Error adding group to user.'), $query);
	}

	function user_add_group($uid, $gid) {
		$user_groups_table = SQL_PREFIX . 'user_groups';		

		/*ingresamos la nueva selección de grupos para el usuario*/
	...
	function create_event($cid, $uid, $subject, $description, $readonly,
			$catid = false, $lugar, $invitados, $archivo_adjunto)
	{
		...
		$query = "INSERT INTO `" . SQL_PREFIX . "events`\n"
			...
			."$fmt_readonly, $catid,'$lugar','$invitados','$archivo_adjunto')";
	...
	function modify_event($eid, $subject, $description, $readonly, $catid = false, $lugar, $invitados, $archivo_adjunto)
	{
		$fmt_readonly = asbool($readonly);
		...
		if($archivo_adjunto!=NULL){
			$query.=",`archivo_adjunto`='$archivo_adjunto'";			
		}
		$query.="WHERE eid='$eid'";
	...
	function getUsuariosPorCalendario($calendario_id)
	{
		$query = "SELECT u.email,u.nombre_completo FROM phpc_users u, phpc_user_groups ug, phpc_groups g, phpc_calendars c WHERE g.gid=ug.gid AND u.uid=ug.uid AND c.cid=g.cid AND c.cid=$calendario_id";

		$sth = $this->dbh->query($query) or $this->db_error('No se encontraron usuarios en el calendario.', $query);

		$emails = array();
		while($email = $sth->fetch_assoc()) {
			$emails[] = $email;
		}
		return $emails;
	}

	function getUltimoIdEvento()
	{
		$query = "SELECT max(eid)+1 as nuevoID FROM ".SQL_PREFIX."events;";
		$sth = $this->dbh->query($query) or $this->db_error('Error:', $query);
	  $data = $sth->fetch_assoc();
	  return $data['nuevoID'];
	}

	function getPlanTrabajo($calendario_id){
		$query = "SELECT plan_trabajo FROM ".SQL_PREFIX."calendars WHERE cid=".$calendario_id.";";
		$sth = $this->dbh->query($query) or $this->db_error('Error:', $query);
	  $data = $sth->fetch_assoc();

...
require_once("$phpc_includes_path/login.php");
...
function userMenu()
{
	global $action, $phpc_user, $phpc_prefix; /*referenciamos a la variable global*/  
	/*si el usuario ha iniciado sesion mostramos las siguientes opciones*/
	$welcome="";
    ...
    if(!empty($_SESSION["{$phpc_prefix}uid"]))
	 ...
	 $html = tag('div', attributes('class="phpc-logged ui-widget-content"'), $welcome, $span);

	 if($action != 'settings' & !empty($_SESSION["{$phpc_prefix}uid"]))
		menu_item_append($span, "<span class='button modificar'>".__('Settings')."</span>", 'settings');
		
	 if(is_user()) {
		menu_item_append($span, "<span class='button borrar'>".__('Log out')."</span>", 'logout',
				array('lasturl' =>
					phpc_html_escape(rawurlencode($_SERVER['QUERY_STRING']))));
	 }else {
		menu_item_append($span, "<span class='button agregar'>".__('Log in')."</span>", 'login',
				array('lasturl' =>
					phpc_html_escape(rawurlencode($_SERVER['QUERY_STRING']))));
	 }
	 return $html;
...
function navbar()
{
	...
	if(is_admin() && $action != 'plantrabajo') {
		menu_item_append($html, 'VER PLAN DE TRABAJO<img src="static/images/ico_archivo_adjunto.png" width="20px" height="20px" align="absmiddle">', 'plantrabajo');
	}
...
function init_config_options() {
	...
	return array( 
			...
			array('language', __('Default Language'), PHPC_DROPDOWN, $languages),
			array('date_format', __('Date Format'), PHPC_DROPDOWN, get_date_format_list()),
			array('theme', __('Theme'), PHPC_DROPDOWN, get_theme_list()),
			array('plan_trabajo', 'PLAN DE TRABAJO<img src="static/images/ico_archivo_adjunto.png" width="20px" height="20px" align="absmiddle">', PHPC_FILE, "")
	);
}
...
function display_loggin()
{
 global $phpc_prefix; /*referenciamos a la variable global*/  
 if(!empty($_SESSION["{$phpc_prefix}uid"]))
 { 	
  	 return display_phpc();  	
 }
 ...
 {
 	//mostramos el formulario de loggin cuando no haya iniciado sesion 	
 	return login(); //en login.php
 }
}
...
function get_header_tags($path)
{
	...
	$estilo = "href=\"//ajax.googleapis.com/ajax/libs/jqueryui/$jqueryui_version/themes/$theme/jquery-ui$jq_min.css\"";
	if($theme=="redmond")
	 $estilo = "href=\"$path/jquery-ui.css\"";	

	return array(
			tag('link', attrs('rel="stylesheet"', 'type="text/css"',"href=\"$path/phpc.css\"")),

			  tag('link', attrs('rel="stylesheet"', 'type="text/css"',$estilo)),

			tag('link', attrs('rel="stylesheet"', 'type="text/css"',"href=\"$path/jquery-ui-timepicker.css\"")),
						
			  tag("script", attrs('type="text/javascript"',"src=\"$path/jquery.min.js\""), ''),
			  tag("script", attrs('type="text/javascript"',"src=\"$path/jquery-ui.min.js\""), ''),

			tag('script', attrs('type="text/javascript"'),"var imagePath='$path/images/'"),
			tag('script', attrs('type="text/javascript"',"src=\"$path/phpc.js\""), ''),
			tag("script", attrs('type="text/javascript"',"src=\"$path/jquery.ui.timepicker.js\""), ''),
			tag("script", attributes('type="text/javascript"',"src=\"$path/jquery.hoverIntent.minified.js\""), ''),
			tag("script", attrs('type="text/javascript"',"src=\"$path/jpicker-$jpicker_version$jq_min.js\""), ''),
			tag('link', attrs('rel="stylesheet"', 'type="text/css"',"href=\"$path/jPicker-$jpicker_version$jq_min.css\"")),
		  );
}

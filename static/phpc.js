...
function setSummaryText(title,author,time,description,category, lugar, invitados, archivo_adjunto) {
  ...
  $("#phpc-summary-lugar").html(lugar);
  $("#phpc-summary-invitados").html(invitados);
  if(archivo_adjunto!=null) 
    $("#phpc-summary-archivo_adjunto").html('<img src="static/images/ico_archivo_adjunto.png">');
  else
    $("#phpc-summary-archivo_adjunto").html('');
}
...
function showSummary(link) {
    ...
    setSummaryText(data.title,data.author,data.time,data.body, data.category, data.lugar, data.invitados, data.archivo_adjunto);    
		showSummaryDiv(link);
	}
	else {

 ...
 */
$default_calendar_id = 0; //el que esté en el index 0
...
$phpc_mailer_path = "$phpc_root_path/PHPMailer";
...
if (!defined('IN_PHPC'))
define('IN_PHPC', true);
//echo "http://www.bestcssbuttongenerator.com/#/C39x7KcVcV";
$phpc_lang = "es";
$logueado=false;
...
if(!empty($_SESSION["{$phpc_prefix}uid"]))
{
 ...
 $calendars = $phpcdb->get_calendars();
	...
	$content = tag('div', attributes('class="php-calendar ui-widget"'),
	  ...
	  tag('h1', attrs('class="ui-widget-header"'), $logueado ? create_dropdown_list(tag('a', attrs("href='$phpc_home_url?phpcid={$phpc_cal->get_cid()}'", 'class="phpc-dropdown-list-title"'), $calendar_title), $list) : "Agenda Digital"),
	   display_loggin() /*display_phpc()*/ /*en calendar.php linea 629 se inicia formulario de logueo*/			
	);
